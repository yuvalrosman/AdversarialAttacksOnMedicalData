...
import utils
import dataset_prepare_ben_yuval
...
def modelnet_params(network_task, config=None):
  ...
  params.full_accuracy_test = {'dataset_folder': params.datasets2use['test'][0],
                               'labels': dataset_prepare_ben_yuval.model_net_labels,
...
def cubes_params(network_task, config = None):
  ...
  params.full_accuracy_test = {'dataset_folder': params.datasets2use['test'][0],
                               'labels': dataset_prepare_ben_yuval.cubes_labels,
                               }
...
def shrec11_params(split_part, network_task, config = None):
  ...
  params.full_accuracy_test = {'dataset_folder': params.datasets2use['test'][0],
                               'labels': dataset_prepare_ben_yuval.shrec11_labels}
...
def medshapeset_minimized_params():
  params = set_up_default_params('classification', 'medShapeNet_minimized', 0)
  params.n_classes = 10
  params.seq_len = 2500
  params.min_seq_len = int(params.seq_len / 2)

  params.datasets2use['train'] = ['datasets_processed/MedShapeNet_20_50_12500_minimized/train/*.npz']
  params.datasets2use['test']  = ['datasets_processed/MedShapeNet_20_50_12500_minimized/test/*.npz']

  params.train_data_augmentation = {'rotation': 360}

  params.full_accuracy_test = {'dataset_expansion': params.datasets2use['test'][0],
                               'labels': dataset_prepare_ben_yuval.medshapenet_labels_minimized}
