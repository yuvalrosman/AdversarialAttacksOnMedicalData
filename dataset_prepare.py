...
medshapenet_labels = [
  'aorta',         'duodenum',        'vertebrae_L1',    'skull',           'femur_left',
  'femur_right',   'kidney_right',    'kidney_left',    'liver',           'stomach',
  'spleen',        'vertebrae_T6',    'colon',          'brain',           'pancreas',
  'uterus',        'tumoredKidney',   'trachea',        'tumoredbrain',    'gallbladder', 'sacrum']

medshapenet_shape2label = {v: k for k, v in enumerate(medshapenet_labels)}

medshapenet_labels_minimized = [
  'aorta',         'skull',           'kidney_right',     'colon',       'brain',
  'uterus',        'tumoredKidney',    'tumoredbrain',    'gallbladder', 'sacrum']


medshapenet_minimized_shape2label = {v: k for k, v in enumerate(medshapenet_labels_minimized)}


def match_string_in_list(strings, file_name):
  matched_string = None
  for string in strings:
    if string in file_name:
      # Check if the current string is longer than the previously matched string
      if matched_string is None or len(string) > len(matched_string):
...
def get_labels(dataset_name, mesh, file, fn2labels_map=None):
  ...
  if dataset_name.startswith('coseg') or dataset_name == 'human_seg_from_meshcnn':
    ...
    elif dataset_name.lower().startswith('medshapenet'):
      model_name = match_string_in_list(medshapenet_labels, file)
      model_label = medshapenet_shape2label[model_name]
    elif dataset_name.lower().startswith('medshapenet'):
      model_name = match_string_in_list(medshapenet_labels,file)
      model_label = medshapenet_shape2label[model_name]
...
def prepare_medshapenet(labels2use=medshapenet_labels,
                  path_in='datasets_raw/MedShapeNet_20_50/',
                  p_out='datasets_processed/MedShapeNet_20_50_12500'):
  dataset_name = 'MedShapeNet'
  if not os.path.isdir(p_out):
    os.makedirs(p_out)
  ...
  for i, name in enumerate(labels2use):
    ...
    for part in ['test', 'train']:
      pin = path_in + name + '/' + part + '/'
      prepare_directory(dataset_name, pathname_expansion=pin + '*.stl',
                                     p_out=p_out, add_labels=labels2use, fn_prefix=part + '_', n_target_faces=[12500],
                                     classification=False)
...
def prepare_one_dataset(dataset_name):
  ...
  if dataset_name == 'datasets_raw/medshapenet_20_50':
    prepare_medshapenet()
...
if __name__ == '__main__':
  ...
  if len(sys.argv) != 2:
    print('Use: python dataset_prepare_walker.py <dataset name>')
    print('For example: python dataset_prepare_walker.py cubes')
    print('Another example: python dataset_prepare_walker.py all')
  else:
    print("step 1: \n")
